...
# Heuristic value
def heuristic(cs, g): # Current state, Goal
    h = 0 # Hamming distance
    for i, j in zip(cs, g.state):
        for k, l in zip(i, j):
            if not (k == l):
                h = h + 1
    return h
...
def generate(c, g):
    ...
    if not x == 0: # Up
        ...
        child = boardState(nState, c.n + 1, c, heuristic(nState, g))
        if not check(child):
...
S = boardState(initState, c = heuristic(initState, G))
